...
export const InviteAcceptancePage: React.FC = () => {
  ...
  const { signIn, refreshClaims } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Call Cloud Function to activate account and set password
      const acceptInvitationFn = httpsCallable(functions, 'acceptInvitation');
      await acceptInvitationFn({
        token: invitation.token,
        newPassword: password,
        ownerName: ownerName.trim() || invitation.ownerName,
      });

      // 2. Sign in to the newly activated account
      await signIn(invitation.ownerEmail, password);
      
      // 3. Refresh claims to pick up cafe_owner role
      await refreshClaims();
