...
export const LiveOrdersTab: React.FC<Props> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Assistance Alert Banners — call_waiter & bill_request */}
      {unresolvedAlerts.length > 0 && (
        <div className="space-y-2">
          {unresolvedAlerts.map(alert => {
            const isBillRequest = alert.type === 'bill_request';
            return (
              <div
                key={alert.id}
                className={`border-2 p-4 rounded-2xl flex items-center justify-between gap-4 animate-pulse shadow-lg text-stone-100 ${
                  isBillRequest
                    ? 'bg-orange-950/80 border-orange-500/80 shadow-orange-500/10'
                    : 'bg-amber-950/80 border-amber-500/80 shadow-amber-500/10'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-xl font-bold ${
                    isBillRequest ? 'bg-orange-500 text-stone-950' : 'bg-amber-500 text-stone-950'
                  }`}>
                    {isBillRequest
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className={`font-bold text-sm ${
                        isBillRequest ? 'text-orange-300' : 'text-amber-300'
                      }`}>
                        {isBillRequest ? '💳 Bill Requested!' : '🔔 Waiter Assistance!'}
                      </span>
                    ...
                    <p className="text-xs text-stone-300 mt-0.5">
                      {isBillRequest
                        ? `${alert.tableLabel || `Table ${alert.tableId}`} is ready to pay`
                        : `${alert.tableLabel || `Table ${alert.tableId}`} called for a waiter`
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-1.5 shadow transition shrink-0 ${
                    isBillRequest
                      ? 'bg-orange-500 hover:bg-orange-400 text-stone-950'
                      : 'bg-amber-500 hover:bg-amber-400 text-stone-950'
                  }`}
                >
                  <UserCheck className="w-4 h-4" /> {isBillRequest ? 'Acknowledged' : 'Resolve'}
                </button>
              </div>
            );
          })}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-stone-800 pb-3">
        ...
        <span className="text-xs text-stone-400 font-medium">
          {selectedStatusFilter === 'history'
            ? `${orders.filter(o => ['Completed', 'Cancelled'].includes(o.status)).length} closed orders`
      ...
      {filteredOrders.length === 0 ? (
        <div className="bg-stone-900/60 border border-stone-800/80 rounded-2xl p-12 text-center text-stone-400 space-y-3">
          ...
          <h3 className="text-base font-bold text-stone-200">
            {selectedStatusFilter === 'history' ? 'No Closed Orders Yet' : 'No Orders in this View'}
          </h3>
          <p className="text-xs text-stone-500 max-w-sm mx-auto">
            {selectedStatusFilter === 'history'
              ? 'Completed and cancelled orders will appear here for reference once orders have been settled.'
