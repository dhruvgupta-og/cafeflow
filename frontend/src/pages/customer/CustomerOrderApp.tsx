...
export const CustomerOrderApp: React.FC = () => {
  ...
  // Session order history (for repeat ordering)
  const SESSION_KEY = `cafeflow_session_${cafeId}_${tableId}`;
  ...
  useEffect(() => {
    if (menuItems.length === 0 || aiSuggestions.length > 0) return;
    const fetchAiSuggestions = async () => {
      ...
      try {
        ...
        const topItems = menuItems.slice(0, 12).map(i => i.name).join(', ');
        const result = await ai.models.generateContent({
          model: 'gemini-2.0-flash',
          contents: `You are a helpful cafe menu assistant. Given these menu items: ${topItems}. Suggest exactly 3 popular item names from this list that customers love most. Respond ONLY with a JSON array of 3 strings, no explanation. Example: ["Item A","Item B","Item C"]`
        });
        const text = result.text?.trim() || '[]';
        const match = text.match(/\[.*\]/s);
        if (match) {
          const parsed = JSON.parse(match[0]);
          if (Array.isArray(parsed)) setAiSuggestions(parsed.slice(0, 3));
        }
      } catch (e) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const updated = Array.from(new Set([...itemNames, ...sessionHistory])).slice(0, 20);
  ...
  const handleRequestBill = async () => {
    if (!cafeId || requestingBill) return;
    setRequestingBill(true);
    try {
      const alertId = `bill-req-${Date.now()}`;
      await setDoc(doc(db, `cafes/${cafeId}/alerts`, alertId), {
        id: alertId,
        cafeId,
        tableId: tableId || 'Walk-in',
  ...
  // Reorder items from session history
  const handleReorder = (itemName: string) => {
    const item = menuItems.find(i => i.name === itemName);
    if (item) handleOpenItemModal(item);
  };
      ...
      {activeTab === 'menu' ? (
        <div className="max-w-lg mx-auto px-4 mt-4 space-y-4">
          ...
          {(aiLoading || aiSuggestions.length > 0) && (
            <div className="bg-gradient-to-r from-indigo-950/80 to-amber-950/60 border border-indigo-500/30 rounded-2xl p-3.5">
              <div className="flex items-center gap-2 mb-2">
                <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                <span className="text-xs font-bold text-amber-300">AI-Recommended for You</span>
                <span className="text-[9px] text-indigo-400 bg-indigo-950 border border-indigo-800 px-1.5 py-0.5 rounded-full font-mono ml-auto">Gemini AI</span>
              </div>
              {aiLoading ? (
                <div className="flex gap-2">
                  {[1,2,3].map(i => <div key={i} className="h-7 flex-1 bg-stone-800 rounded-xl animate-pulse" />)}
                </div>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {aiSuggestions.map(name => (
                    <button
                      ...
                      className="px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-300 rounded-xl text-xs font-semibold transition"
                    >
                      ✨ {name}
                    </button>
                  ))}
          ...
          {/* Session Reorder History */}
          {sessionHistory.length > 0 && (
            <div className="bg-stone-900 border border-stone-800 rounded-2xl p-3.5">
              <div className="flex items-center gap-2 mb-2">
                <RotateCcw className="w-3.5 h-3.5 text-stone-400" />
                <span className="text-xs font-bold text-stone-300">Order Again</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {sessionHistory.slice(0, 5).map(name => (
                  <button
                    key={name}
                    onClick={() => handleReorder(name)}
                    className="px-3 py-1.5 bg-stone-800 hover:bg-stone-700 border border-stone-700 text-stone-300 rounded-xl text-xs font-semibold transition"
                  >
                    ↩ {name}
                  </button>
                ))}
          ...
          {activeOrders.length > 0 && (
            <div className="bg-stone-900 border border-stone-800 rounded-2xl p-4 space-y-3">
              <h3 className="text-sm font-bold text-stone-200 flex items-center gap-2">
                <Receipt className="w-4 h-4 text-amber-400" /> Ready to Pay?
              </h3>
              {billRequested ? (
                <div className="p-3 bg-emerald-950/80 border border-emerald-500/60 rounded-xl text-xs text-emerald-300 flex items-center gap-2">
                  ...
                  Bill request sent! A staff member will bring your bill shortly.
                </div>
              ) : (
                <button
                  onClick={handleRequestBill}
                  disabled={requestingBill}
                  className="w-full py-3 rounded-xl bg-stone-800 hover:bg-stone-700 border border-stone-700 text-stone-200 font-bold text-sm flex items-center justify-center gap-2 transition"
                >
                  <Receipt className="w-4 h-4 text-amber-400" />
                  {requestingBill ? 'Sending...' : 'Request Bill'}
                </button>
              )}
            </div>
          )}
