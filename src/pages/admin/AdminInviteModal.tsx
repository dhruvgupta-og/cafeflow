...
export const AdminInviteModal: React.FC<Props> = ({ cafe, isOpen = true, onClose, existingInvitation }) => {
  ...
  const handleSimulateSendEmail = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => setEmailSentSimulated(false), 5000);
    }, 800);
  };
      ...
      <div className="bg-stone-900 border border-stone-800 rounded-3xl w-full max-w-xl text-stone-100 shadow-2xl overflow-hidden flex flex-col my-6">
        ...
        <div className="p-6 space-y-5 overflow-y-auto max-h-[75vh]">
          {loading ? (
            ...
            <>
              {/* Notification Banner when simulated email sent */}
              {emailSentSimulated && (
                <div className="p-3 bg-emerald-950 border border-emerald-500/80 rounded-2xl text-xs text-emerald-200 flex items-center gap-2 shadow-lg animate-fadeIn">
                  <Check className="w-4 h-4 text-emerald-400 shrink-0" />
                  <span>
                    Invitation email successfully sent to <strong>{cafe.email}</strong>!
                  </span>
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-stone-800">
                ...
                >
                  ...
                  {sendingEmail ? 'Dispatching Email...' : 'Send Invitation Email'}
                </button>
