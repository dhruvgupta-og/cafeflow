...
export const AdminAddCafeModal: React.FC<Props> = ({ isOpen, onClose, onSuccess, onOpenInviteModal }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Generate clean slug cafe ID
      const baseSlug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'cafe';
      const cafeId = `cafe-${baseSlug}-${Math.random().toString(36).substring(2, 6)}`;
      const tempPass = 'CafeOwner!' + Math.floor(1000 + Math.random() * 9000);
      ...
      // 1. Create Cafe in Firestore
      await setDoc(doc(db, 'cafes', cafeId), newCafe);

      // 2. Provision initial tables
      const initialTables = [
        { id: 'T1', label: 'Table 1', status: 'free' },
        { id: 'T2', label: 'Table 2', status: 'free' },
        { id: 'T3', label: 'Table 3', status: 'free' },
        { id: 'T4', label: 'Table 4', status: 'free' },
      ];
      for (const t of initialTables) {
        await setDoc(doc(db, `cafes/${cafeId}/tables`, t.id), t);
      }

      // 3. Provision sample initial category & item
      const initialCatId = 'cat-beverages';
      await setDoc(doc(db, `cafes/${cafeId}/menuCategories`, initialCatId), {
        id: initialCatId,
        name: 'Beverages & Coffee',
        sortOrder: 1
      });
      await setDoc(doc(db, `cafes/${cafeId}/menuCategories/${initialCatId}/items`, 'item-signature-coffee'), {
        id: 'item-signature-coffee',
        categoryId: initialCatId,
        name: `${name} Signature Coffee`,
        description: 'House roasted blend crafted fresh to order.',
        price: 4.50,
        photoUrl: 'https://images.unsplash.com/photo-1514432324607-a09d9b4aefdd?w=600&auto=format&fit=crop&q=80',
        isVeg: true,
        isAvailable: true,
        addOns: [{ name: 'Oat Milk', price: 0.75 }, { name: 'Extra Shot', price: 1.25 }]
      });

      // 4. Try creating Auth user or fallback doc
      try {
        await signUp(email, tempPass, 'cafe_owner', cafeId, ownerName);
      } catch (authErr) {
        console.warn('Auth user creation note:', authErr);
      }

      // 5. Generate Invitation Token & link for owner onboarding
      let inviteUrl = '';
      try {
