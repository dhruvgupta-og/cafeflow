...
export const CafeLogin: React.FC = () => {
  ...
  const handleQuickDemo = (targetCafeId: string, role: 'cafe_owner' | 'staff', demoEmail: string) => {
    impersonateRole(role, targetCafeId, demoEmail);
    navigate(`/cafe/${targetCafeId}`);
  };
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4">
        <div className="bg-stone-900 py-8 px-6 shadow-xl border border-stone-800 sm:rounded-2xl sm:px-10">
          ...
          <div className="mt-6 pt-6 border-t border-stone-800 space-y-2">
            <span className="text-[11px] font-semibold uppercase text-stone-400 block text-center mb-2">
              ⚡ Instant Demo Profiles
            </span>

            <button
              type="button"
              onClick={() => handleQuickDemo('cafe-velvet-roast', 'cafe_owner', 'elena@velvetroast.com')}
              className="w-full py-2 px-3 bg-stone-800 hover:bg-stone-750 border border-stone-700 rounded-lg text-xs font-medium text-stone-200 transition-colors flex items-center justify-between"
            >
              <span className="flex items-center gap-2">
                <Coffee className="w-3.5 h-3.5 text-amber-400" /> Velvet Roast (Owner)
              </span>
              <span className="text-amber-400 text-[10px]">Launch &rarr;</span>
            </button>

            <button
              type="button"
              onClick={() => handleQuickDemo('cafe-matcha-haven', 'cafe_owner', 'kenji@matchahaven.com')}
              className="w-full py-2 px-3 bg-stone-800 hover:bg-stone-750 border border-stone-700 rounded-lg text-xs font-medium text-stone-200 transition-colors flex items-center justify-between"
            >
              <span className="flex items-center gap-2">
                <Coffee className="w-3.5 h-3.5 text-emerald-400" /> The Matcha Haven
              </span>
              <span className="text-emerald-400 text-[10px]">Launch &rarr;</span>
            </button>
          </div>
